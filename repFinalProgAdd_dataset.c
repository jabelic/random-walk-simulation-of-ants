    ...
    for (int s = 1; s <= 100; s++){
        ...
        for (int m = 0; m < M; m++)
        {
            ...
            for (int k = 0; k < N; k++)
            {
                ...
                if (i == X_FEED && j == Y_FEED)//餌にありつけた判定
                {
                    x[m] = i;//餌の位置を最終位置とし、そこでランダムウォークは終了。
                    y[m] = j;
                    AntFindFeed[m] += 1;
                    break;
                }
        ...
        printf("s = %d, f = %d ", s, f);
        printf("p=%lf％,", p);
        int AntCouldEet = 0;//何匹の蟻が餌にありつけたかを格納する変数
        for (int a = 0; a < M; a++)
        {
            AntCouldEet += AntFindFeed[a];
        }
        printf("餌にありつけた蟻の数は%d匹\n", AntCouldEet);
        rate[list_numeric] = p;//帰巣成功確率を格納
        list_numeric += 1;
    }
    for (i = 0; i < 100; i++)
    {
